...
> 本文主要以 OpenJDK 8 / HotSpot 源码布局为例。JDK 10 之后源码目录有调整, 例如部分路径会从 `hotspot/src/share/vm/...` 变为 `src/hotspot/share/...`。
...
```
#### 4. `Threads::create_vm()`
位于 OpenJDK 8 的 `hotspot/src/share/vm/runtime/thread.cpp`。如果阅读的是新版 JDK 源码, 对应路径通常是 `src/hotspot/share/runtime/thread.cpp`。
