...
源码位置: `hotspot/src/share/vm/oops/oopsHierarchy.hpp`

在HotSpot中, `oop` 本身可以先简单理解为 `oopDesc*`。也就是说 Java 堆里的对象并不是 C++ 里直接 new 出来的某个业务类, 而是一块由 HotSpot 解释的内存, 这块内存的起始部分按 `oopDesc` 的格式存放对象头。
...
| :---: | :--- |
| `oop` | 普通对象指针的基类视角 |
| `instanceOop` | Java普通实例对象 |
| `arrayOop` | Java数组对象的基类视角 |
| `objArrayOop` | 引用类型数组, 如 `Object[]`、`String[]` |
| `typeArrayOop` | 基本类型数组, 如 `int[]`、`long[]` |

这些类型的差异主要体现在对象头之后的数据区如何解释。普通对象的数据区是实例字段; 数组对象除了对象头之外还会有数组长度, 后面才是连续的数组元素。

### 3. Java对象内存布局

一个普通 Java 对象在堆中的结构可以先粗略理解成:

```text
+--------------------+
| mark word          |  锁状态、hash、GC年龄等信息
+--------------------+
| klass pointer      |  指向 Klass 元数据, 压缩后通常为4字节
+--------------------+
...
这里的 `klass pointer` 是运行时识别对象类型的入口。通过它可以找到对应的 `Klass` 元数据, 进而拿到方法表、字段布局、父类、接口等信息。

### 4. oop、Handle与GC

直接拿 `oop` 指针时需要注意一个问题: Java堆对象在GC过程中可能移动。HotSpot内部如果跨越可能发生GC的点保存对象引用, 通常不会长期裸持有 `oop`, 而是通过 `Handle`、`JNIHandleBlock` 等机制间接保存。

简单理解:

* `oop`: 指向 Java 堆对象的原始指针视角
* `Handle`: 对 `oop` 的一层间接包装, 方便 GC 移动对象后更新引用
* `jobject`: JNI层看到的对象引用, 内部也不是直接暴露裸 `oop`

这也是为什么JNI层不能把 `jobject`、`jclass` 直接当成 HotSpot 内部的 `oop` 或 `Klass*` 来理解。JNI给的是一层稳定的访问接口, HotSpot内部再通过句柄机制找到真实对象。

### 5. 对象创建时发生了什么

以 `new Entity()` 为例, 可以先按下面这条线理解:

1. 类加载阶段生成 `InstanceKlass`, 保存 `Entity` 的类元信息
2. 执行 `new` 字节码时, 根据 `InstanceKlass` 中记录的对象大小申请堆内存
3. 初始化对象头, 写入 mark word 和 klass pointer
4. 将实例字段初始化为默认值
5. 调用 `<init>` 构造方法完成 Java 层初始化

这里要区分两个初始化:

* 对象内存的零值初始化: 分配对象之后字段先是默认值
* 构造方法初始化: 执行 `<init>` 中的 Java 代码
